...
export default function OnlineResultPanel({ result, label, mode, numTasks, numMachines, highlight }) {
  if (!result) return null;

  const { perMachine, overall } = computeUtilization(result.schedule, numMachines, result.makespan);
    ...
    <section className={`card result-card ${highlight ? 'result-highlight' : ''}`}>
      ...
      <div className="util-grid">
        {perMachine.map((u, i) => (
          <div key={i} className="util-item">
            ...
            <div className="util-bar-track">
              <div className="util-bar-fill" style={{ width: `${u.toFixed(1)}%` }} />
            </div>
        ...
        <div className="util-item util-overall">
          ...
          <div className="util-bar-track">
            <div className="util-bar-fill util-bar-overall" style={{ width: `${overall.toFixed(1)}%` }} />
          </div>
      ...
      <GanttChart schedule={result.schedule} numMachines={numMachines} numTasks={numTasks} />

      <OnlineStepTable steps={result.steps} mode={mode} />
    </section>
  );
}
